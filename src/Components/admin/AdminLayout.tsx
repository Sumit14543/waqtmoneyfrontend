...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC] flex text-slate-800 font-sans relative">
      {/* Sidebar - Desktop (Fixed sticky sidebar) */}
      <aside className="hidden lg:flex flex-col w-64 h-screen sticky top-0 bg-[#0B132B] text-slate-300 shrink-0 select-none z-40">
        {/* Brand Header */}
        ...
        {/* Navigation Items (Scrollable inside sidebar if needed) */}
        ...
        {/* User Profile Footer */}
        <div className="p-4 border-t border-slate-800/80 bg-[#070D1E]/60 shrink-0">
          <div className="flex items-center gap-3 mb-3 px-2">
            <div className="h-9 w-9 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-300 shrink-0">
              <User size={18} />
            </div>
      ...
      <div className="flex-grow flex flex-col min-w-0">
        {/* Top Clock Bar (Sticky) */}
        <header className="sticky top-0 z-30 bg-white border-b border-slate-200/80 px-6 py-3 flex items-center justify-between shadow-2xs">
          <div className="flex items-center gap-3">
            <button
